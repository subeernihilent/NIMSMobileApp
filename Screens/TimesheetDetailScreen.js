...
import { FlatList, StyleSheet, Text, TouchableOpacity, View, ActivityIndicator, Alert, AsyncStorage } from 'react-native';
...
export default function TimesheetDetailScreen({ navigation, route }) {
    ...
    const onSubmitClick = () => {
        setLoading(true);
        let batch = db.batch();
        let timsheetRef = db.collection("Timesheet").doc(userEmail);
        timsheetRef.get().then((docSnapshot) => {
            if (docSnapshot.exists) {
                timsheetRef.onSnapshot((doc) => {
                    timsheetRef.update({
            ...
            let weekDate = db.collection("Week").doc(userEmail);
            batch.update(weekDate, {
                weekdays: [{
                    status: true,
                    week: route.params.dateName
                }]
            })
